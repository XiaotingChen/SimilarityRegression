...
def PercentIdentityVect(
    aln_l, aln_s, Norm="L", SmoothingWindow_3=False, subMat=ReturnBlossum62Dict()
):
    ...
    return (
        AAPercIdentityVect.tolist(),
        AAAvgScoreVect.tolist(),
    )  # Deimplemented: AAMaxScoreVect
...
def AlignDBDArrays(i_x, i_y, ByPosNorm="L"):
    ...
    for start in range(0, len(L_Array) - span + 1):
        ...
        numID, pctID_OverlappingSites = AlnmtPctID(
            "".join(L_Array), "".join(currentaln)
        )
        IDs.append(numID)
    ...
    o["ByPos.PctID"], o["ByPos.AvgB62"] = PercentIdentityVect(
        L_Array, S_BestAln, Norm=ByPosNorm, SmoothingWindow_3=False
    )
    o["ByPos.PctID.Smooth3"], o["ByPos.AvgB62.Smooth3"] = PercentIdentityVect(
        L_Array, S_BestAln, Norm=ByPosNorm, SmoothingWindow_3=True
    )
