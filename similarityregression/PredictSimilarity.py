...
def ScoreAlignmentResult(resultDict, scoreDict, applyidenticalRule=True):
    ...
    if scoreDict["Model.Class"] == "SequenceIdentity":
        ...
        ByPos = np.array(
            resultDict["ByPos." + scoreDict["SR.Features"].replace("_", ".")]
        )
        # Normalize to features (f)
        f = (ByPos - scoreDict["SR.FeatureScales.mean"]) / scoreDict[
            "SR.FeatureScales.sd"
        ]
...
def AlignAndScore_DictPairs(
    t,
    SRModel,
    OutputClasses=["HSim", "Dis"],
):
    ...
    # Score alignment
    aln_score = ScoreAlignmentResult(resultDict=aln_result, scoreDict=SRModel)
...
def SeqDictIterator_ParseIdentical2Results(d, SRModel):
    results = []
    import itertools
